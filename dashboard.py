...
def load_model_and_artifacts():
    ...
    try:
        # Load hyperparameters from JSON to get correct model architecture
        import json
        ...
        if os.path.exists(hyperparams_file):
            ...
            st.success(
                f"✓ Loaded hyperparameters: {hidden1}-{hidden2} architecture, dropout={dropout_rate:.3f}")
        else:
            # Fallback to defaults if JSON not found
            hidden1, hidden2, dropout_rate = 128, 64, 0.3
            st.warning(
                "⚠️ Using default architecture (128-64). Run notebook to save optimal hyperparameters.")
        ...
        model = PitcherClassifier(
            num_features, num_pitchers, hidden1=hidden1, hidden2=hidden2, dropout_rate=dropout_rate)
        ...
        if os.path.exists('outputs/confusion_mass_matrix.npy'):
            confusion_matrices['C'] = np.load(
                'outputs/confusion_mass_matrix.npy')
            confusion_matrices['m'] = np.load('outputs/off_diagonal_mass.npy')
            confusion_matrices['Sim'] = np.load(
                'outputs/similarity_matrix_weighted_symmetric.npy')
            confusion_matrices['uniqueness'] = np.load(
                'outputs/pitcher_uniqueness.npy')
            confusion_matrices['pitcher_names'] = np.load(
                'outputs/pitcher_names.npy', allow_pickle=True)
            st.success("✓ Loaded confusion-based similarity matrices")
        else:
            st.warning(
                "⚠️ Confusion matrices not found. Run the similarity computation cells in the notebook first.")
...
def compute_pitcher_similarity(model, scaler, label_encoder, df, pitcher_name, confusion_matrices=None, filter_pitch_type=None, filter_count=None):
    ...
    if filter_pitch_type:
        pitch_type_cols = [
            col for col in pitcher_pitches.columns if col.startswith('pitch_type_')]
        if f'pitch_type_{filter_pitch_type}' in pitch_type_cols:
            pitcher_pitches = pitcher_pitches[
                pitcher_pitches[f'pitch_type_{filter_pitch_type}'] == 1]

    if filter_count:
        count_cols = [
            col for col in pitcher_pitches.columns if col.startswith('count_')]
    ...
    pitch_details['predicted_pitcher'] = label_encoder.inverse_transform(
        predictions)
    ...
    for i in range(3):
        pitch_details[f'top_{i+1}_pitcher'] = label_encoder.inverse_transform(
            top3_indices[:, i])
        pitch_details[f'top_{i+1}_prob'] = probabilities[np.arange(
            len(probabilities)), top3_indices[:, i]]
...
def plot_network_graph(df, pitcher_name, similarity_scores, distance_matrix, metric_name="Cosine", top_n=30, all_pitchers_in_matrix=None):
    ...
    if all_pitchers_in_matrix is not None:
        pitcher_indices = [all_pitchers_in_matrix.index(
            p) for p in top_pitchers if p in all_pitchers_in_matrix]
    else:
        all_pitchers = sorted(df['player_name'].unique())
        pitcher_indices = [all_pitchers.index(
            p) for p in top_pitchers if p in all_pitchers]
    ...
    for i, j, dist in edges:
        ...
        norm_dist = (dist - min_dist) / (max_dist -
                                         min_dist) if max_dist != min_dist else 0.5
    ...
    node_colors = ['red' if top_pitchers[i] ==
                   pitcher_name else 'lightblue' for i in range(len(top_pitchers))]
    node_sizes = [25 if top_pitchers[i] ==
                  pitcher_name else 15 for i in range(len(top_pitchers))]
    ...
    node_trace = go.Scatter(
        ...
        hoverinfo='text',
        hovertext=[
            f'{p}<br>Pitches: {len(df[df["player_name"]==p])}' for p in top_pitchers],
        showlegend=False
    )
...
def calculate_uniqueness_percentile(uniqueness, all_pitcher_data, df, label_encoder):
    ...
    for pitcher_name in label_encoder.classes_:
        ...
        if len(pitcher_data) == 0:
            continue
        # Simplified - in production would calculate for each
        uniqueness_scores.append(uniqueness)

    # Calculate percentile (what % of pitchers have lower uniqueness)
    if len(uniqueness_scores) > 0:
        percentile = (sum(1 for s in uniqueness_scores if s <
                      uniqueness) / len(uniqueness_scores)) * 100
        return percentile
    return 50.0
...
def plot_pitch_type_similarity_heatmap(pitch_details, label_encoder):
    ...
    pitch_type_cols = [col for col in pitch_details.columns if col.startswith(
        'pitch_type_') and pitch_details[col].sum() > 0]

    if len(pitch_type_cols) == 0:
        return None

    # Get top predicted pitchers
    top_pitchers = pitch_details['top_1_pitcher'].value_counts().head(
        10).index.tolist()
...
def main():
    ...
    st.markdown(
        "Analyze pitcher similarities based on pitch characteristics and game context.")
    ...
    mode = st.sidebar.radio(
        "Mode", ["Pitcher Similarity", "Custom Pitch Prediction"])
    ...
    if mode == "Pitcher Similarity":
        ...
        # Pitch type filter
        pitch_type_cols = [col.replace(
            'pitch_type_', '') for col in df.columns if col.startswith('pitch_type_')]
        pitch_type_filter = st.sidebar.selectbox(
            "Pitch Type", ["All"] + pitch_type_cols)

        # Count filter
        count_cols = [col.replace('count_', '')
                      for col in df.columns if col.startswith('count_')]
        ...
        top_similar_pitchers = sorted(
            similarity_scores.items(), key=lambda x: x[1], reverse=True)[:20]
        top_pitcher_names = [selected_pitcher] + \
            [p for p, _ in top_similar_pitchers]
        ...
        with st.spinner("Computing Euclidean distance matrix..."):
            ...
            for p in all_pitchers:
                p_data = df[df['player_name'] == p].drop(
                    ['player_name'], axis=1)
                if len(p_data) > 0:
            ...
            for i in range(n_pitchers):
                for j in range(n_pitchers):
                    if i != j:
                        euclidean_dist_matrix[i][j] = np.linalg.norm(
                            pitcher_vectors[i] - pitcher_vectors[j])
            ...
            for i, p in enumerate(all_pitchers):
                if i != pitcher_idx:
                    euclidean_similarities[p] = 1 - (
                        euclidean_dist_matrix[pitcher_idx][i] / euclidean_dist_matrix.max())
        ...
        with col2:
            ...
            percentile = (sum(1 for u in all_uniqueness if u < uniqueness) /
                          len(all_uniqueness)) * 100 if all_uniqueness else 50
        ...
        with col3:
            avg_similarity = np.mean(list(similarity_scores.values())[
                                     :top_n]) if similarity_scores else 0
        ...
        if similarity_scores:
            ...
            with col_sim2:
                ...
                fig_bar_euc = plot_similarity_bar_chart(
                    euclidean_similarities, top_n)
                if fig_bar_euc:
                    fig_bar_euc.update_layout(
                        title=f'Top {top_n} Most Similar (Euclidean)')
                    st.plotly_chart(fig_bar_euc, use_container_width=True)

            # Radar chart comparison
            similar_pitcher_names = [p for p, _ in sorted(
                similarity_scores.items(), key=lambda x: x[1], reverse=True)]
            fig_radar = plot_pitch_characteristics_radar(
                df, selected_pitcher, similar_pitcher_names, top_n=3)
            if fig_radar:
                st.plotly_chart(fig_radar, use_container_width=True)

            # Pitch type heatmap
            if len(pitch_details) > 0:
                fig_heatmap = plot_pitch_type_similarity_heatmap(
                    pitch_details, label_encoder)
                if fig_heatmap:
                    st.plotly_chart(fig_heatmap, use_container_width=True)

            # Network graphs
            st.subheader("Similarity Network Visualizations")
            tab_net1, tab_net2 = st.tabs(
                ["Model-Based Network", "Euclidean Network"])
            ...
            with tab_net1:
                ...
                if fig_network:
                    ...
                    st.caption(
                        "Network graph showing asymmetric model-based similarities. Curved edges show A→B may differ from B→A.")
                else:
                    st.info(
                        "No network graph available - insufficient similarity data for this pitcher.")
            ...
            with tab_net2:
                ...
                if fig_network_euc:
                    ...
                    st.caption(
                        "Network graph using Euclidean distance on averaged features.")
                else:
                    st.info(
                        "No network graph available - insufficient similarity data for this pitcher.")
                    st.plotly_chart(fig_network_euc, use_container_width=True)
            ...
            similarity_df = pd.DataFrame(sorted_scores, columns=[
                                         'Pitcher', 'Similarity Score'])
            similarity_df['Rank'] = range(1, len(similarity_df) + 1)
            similarity_df = similarity_df[[
                'Rank', 'Pitcher', 'Similarity Score']]
        ...
        st.markdown(
            "Enter pitch characteristics to see which pitcher it most resembles.")
        ...
        pitch_type_features = [
            col for col in feature_names if col.startswith('pitch_type_')]
        count_features = [
            col for col in feature_names if col.startswith('count_')]
        ...
        with st.form("custom_pitch_form"):
            ...
            with col2:
                ...
                pitch_type_names = [col.replace(
                    'pitch_type_', '') for col in pitch_type_features]
                selected_pitch_type = st.selectbox(
                    "Pitch Type", pitch_type_names)
                ...
                # Count selection
                count_names = [col.replace('count_', '')
                ...
                if 'matchup' in feature_names:
                    feature_values['matchup'] = float(st.selectbox(
                        "Matchup", ["Same Hand (0)", "Opposite Hand (1)"]).split('(')[1][0])

            submit_button = st.form_submit_button("Predict Pitcher")

        if submit_button:
            with st.spinner("Predicting..."):
                top_predictions = predict_custom_pitch(
                    model, scaler, label_encoder, feature_values, feature_names)
            ...
            # Display as table
            pred_df = pd.DataFrame(top_predictions, columns=[
                                   'Pitcher', 'Probability'])
